#[cfg(test)]
mod tests {
    use set_covering_problem::utils::read_input::{InputError,ReadInput};
    use std::fs::File;
    use std::io::Write;
    ...
    fn test_new_with_args() {
        let args = vec!["program".to_string(), "-c".to_string(), "1".to_string()];
        let result = ReadInput::new(args);
        assert!(result.is_ok());
    }

    #[test]
    fn test_get_subset_from_file() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test.txt");
        let mut file = File::create(&file_path).unwrap();
        writeln!(file, "1,4,5,8,9").unwrap();
        writeln!(file, "2,3 ,6").unwrap();
        writeln!(file, "1,  4,0,   7").unwrap();
        ...
        let subsets: Vec<Vec<String>> = ri.read_file_subset().unwrap();
        assert_eq!(subsets,vec![
            vec!["1".to_string(),"4".to_string(),"5".to_string(),"8".to_string(),"9".to_string()],
            vec!["2".to_string(),"3".to_string(),"6".to_string()],
            vec!["1".to_string(),"4".to_string(),"0".to_string(),"7".to_string()]
        ]);
    }
    ...
    fn test_invalid_path_file() {
        let mut args = vec!["program".to_string(), "-c".to_string(), "nonexistent.txt".to_string()];
        let mut ri = ReadInput::new(args.clone()).unwrap();
        let res = ri.read_file_subset();
